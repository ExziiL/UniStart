...
export async function PUT(req: NextRequest) {
	...
	try {
		...
		if (!conversationMongo) {
			return NextResponse.json({ message: "Creating Conversation failed" }, { status: 500 });
		}
		let match = "MATCH ";
		let createRelation = "";
		let createConversation =
			' CREATE ( conversation:Conversation {id: $conversation.id, lastmessage: "", group: $isgroup})';
